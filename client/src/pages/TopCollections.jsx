        ...
        const fetchData = async () => {
            try {
                const response = await axios.get(
                    `http://localhost:4000/api/v1/products?page=${count}&category=Top`
                );
                    ...
                    {filteredProducts.map((element) => {
                        return (
                            <div className='ease-in-out cursor-pointer h-full min-h-[18rem] w-full md:w-[18rem] lg:w-full md:h-[30.5rem] xl:h-[43rem] overflow-hidden relative' key={element.id}>
                                <img alt="team" className="relative flex-shrink-0 viol object-center mb-4  ease-in-out object-cover h-full min-h-[18rem] w-full md:w-[18rem] lg:w-full  md:h-[30.5rem] xl:h-[43rem] hover:scale-110 duration-700 " src={element.images[0].url} />
